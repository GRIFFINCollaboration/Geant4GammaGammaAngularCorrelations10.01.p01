...
void G4DiscreteGammaTransition::SelectGamma()
{
  ...
  if (nGammas > 0) {
    ...
    */
    ...
    _a2               = (aLevel->A2())[iGamma];
    _a4               = (aLevel->A4())[iGamma];
    _a6               = (aLevel->A6())[iGamma];
    _a8               = (aLevel->A8())[iGamma];
    _a10              = (aLevel->A10())[iGamma];
    _maxWTheta        = (aLevel->MaxWTheta())[iGamma];
...
// Evan Rand - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
G4double G4DiscreteGammaTransition::GetThetaFromWTheta(G4double higherLevelEnergy)
{
  G4double  value, WTheta, theta, normy;
  ...
  if(higherLevelEnergy<0) {
    ...
    while (!foundTheta) {
      ...
      WTheta = (1+_a2[hlevelindex]*LegendreP(2,std::cos(theta))+_a4[hlevelindex]*LegendreP(4,std::cos(theta))+_a6[hlevelindex]*LegendreP(6,std::cos(theta))+_a8[hlevelindex]*LegendreP(8,std::cos(theta))+_a10[hlevelindex]*LegendreP(10,std::cos(theta)))*std::sin(theta);
      if(normy <= WTheta) {
        value = theta;
        foundTheta = true;
        break;
