...
:theRadioactiveDecayContainer(theRadioactiveDecayContainer1)
{
  ...
  // Evan Rand - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // Need the multipole of the gamma transitions with mixing ratios
  userMultipoleDataCmd = new G4UIcommand("/grdm/setMultipoleFile",this);
  userMultipoleDataCmd->SetParameter(Z_para);
  userMultipoleDataCmd->SetParameter(A_para);
  userMultipoleDataCmd->SetParameter(FileName_para);

  G4UIparameter*  Ang_para= new G4UIparameter("gs_ang",'i',true);
  Ang_para->SetParameterRange("Ang_isotope >= 0");
  Ang_para->SetGuidance("Ground state angular momentum");

  userMultipoleGSCmd = new G4UIcmdWith3Vector("/grdm/setMultipoleGroundStateSpinAngularMomentum",this);
